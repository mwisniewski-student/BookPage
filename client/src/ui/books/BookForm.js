...
const BookForm = ({ initialValues, onSubmit, authors, books, authorRequestStatus, bookRequestStatus,
    getAuthorList, getBookList, setBookRequestStatus, setAuthorRequestStatus }, { t }) => {
    ...
    const bookSchema = Yup.object().shape({
        title: Yup.mixed().required(t("Book title is required!"))
        ...
        categories: Yup.array().of(Yup.object().shape({
            ...
            value: Yup.mixed().oneOf(categories, t("Categories must be chosen from the list!")).required()
        })).min(1, t("At least one category must be chosen!")),
        publishDate: Yup.date().max(new Date(), t("Book must be published now!"))
            .required(t("Publish date is required!")),
        image: Yup.string().matches(/^https?:\/\/.+\/.+$/, t("Image must be url")),
        numberOfPages: Yup.number().min(1, t("Book must have at least one page!")).required(t("Number of pages is required!")),
        ...
        authorsIds: Yup.array().of(Yup.object().shape({
            ...
            value: Yup.mixed().oneOf(authors.map(author => {
                return author.id
            }), t("Authors must be chosen from the list!")).required()
        ...
        <div>
            {
                ...
                >
                    <Form>
                        <div className="mb-3">
                            <BootstrapForm.Label htmlFor="name">{t('Title')}</BootstrapForm.Label>
                            <Field name="title" className="form-control" id="title" />
                            <ErrorMessage className="text-danger" name="title" component="div" />
                        </div>
                        <div className="mb-3">
                            <BootstrapForm.Label htmlFor="publishDate">{t('Published')}</BootstrapForm.Label>
                        ...
                        <div className="mb-3">
                            <BootstrapForm.Label htmlFor="authorsIds">{t('Authors')}</BootstrapForm.Label>
                        ...
                        <div className="mb-3">
                            <BootstrapForm.Label htmlFor="numberOfPages">{t('Number Of Pages')}</BootstrapForm.Label>

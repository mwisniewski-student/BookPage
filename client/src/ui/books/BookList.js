...
const BookList = (
  { books, getBookList, setBookRequestStatus, bookRequestStatus },
  { t }
) => {
  useEffect(() => {
  ...
  const setStartingOptions = () => {
    const booksSorted = [...books].sort((x, y) =>
  ...
  const sortByNumberOfPages = () => {
    setDisplayedBooks([
      ...displayedBooks.sort((x, y) => y.numberOfPages - x.numberOfPages),
    ]);
  ...
  const sortByNumberOfPagesAscending = () => {
    setDisplayedBooks([
      ...displayedBooks.sort((x, y) => x.numberOfPages - y.numberOfPages),
    ]);
  ...
  const sortByPublishDate = () => {
    setDisplayedBooks([
      ...displayedBooks.sort(
        (x, y) => new Date(x.publishDate) - new Date(y.publishDate)
      ),
    ]);
  ...
  const currentDisplayedBooks = displayedBooks.slice(
    indexOfFirstBook,
    indexOfLastBook
  );
  ...
  return (
    <div>
      ...
      <ButtonGroup aria-label="Filter or Sort" className="mb-3">
        ...
        >
          <Dropdown.Item onClick={sortByNumberOfPages}>
            {t("Number Of Pages")}
          </Dropdown.Item>
          <Dropdown.Item onClick={sortByNumberOfPagesAscending}>
            {t("Number Of Pages(Ascending)")}
          </Dropdown.Item>
          ...
          <Dropdown.Item onClick={sortByPublishDateReverse}>
            {t("By Youngest")}
          </Dropdown.Item>
        </DropdownButton>
      </ButtonGroup>
      ...
      {currentDisplayedBooks
        ? currentDisplayedBooks.map((book) => {
            return (
                  ...
                  <div className="col-md-8">
                    <Card.Body>
                      ...
                      {book.categories.map((category, i) => (
                        <Badge className="me-2" bg="secondary" key={i}>
                          {t(category)}
                        </Badge>
                      ))}

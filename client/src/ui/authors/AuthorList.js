...
const AuthorList = (
  ...
  const setStartingOptions = () => {
    const authorsSortedAndMapped = [...authors]
  ...
  const sortByNumberOfBooks = () => {
    setDisplayedAuthors([
      ...displayedAuthors.sort(
        (x, y) => getNumberOfBooks(y.id) - getNumberOfBooks(x.id)
      ),
    ]);
  ...
  const sortByNumberOfBooksAscending = () => {
    setDisplayedAuthors([
      ...displayedAuthors.sort(
        (x, y) => getNumberOfBooks(x.id) - getNumberOfBooks(y.id)
      ),
    ]);
  ...
  const sortAlphabeticallyReverse = () => {
    setDisplayedAuthors([
      ...displayedAuthors.sort((x, y) =>
        y.name.toLowerCase().localeCompare(x.name.toLowerCase())
      ),
    ]);
  ...
  const sortByBirthDate = () => {
    setDisplayedAuthors([
      ...displayedAuthors.sort(
        (x, y) => new Date(x.birthDate) - new Date(y.birthDate)
      ),
    ]);
  ...
  const sortByBirthDateReverse = () => {
    setDisplayedAuthors([
      ...displayedAuthors.sort(
        (x, y) => new Date(y.birthDate) - new Date(x.birthDate)
      ),
    ]);
  ...
  const currentDisplayedAuthors = displayedAuthors.slice(
    indexOfFirstAuthor,
    indexOfLastAuthor
  );
  ...
  return (
    <div>
      ...
      <ButtonGroup aria-label="Filter or Sort" className="mb-3">
        <Button variant="primary" onClick={handleShowFilterCanvas}>
          <IoOptionsSharp size={25} /> {t("Filter")}
        </Button>
        <DropdownButton
          as={ButtonGroup}
          id="bg-nested-dropdown"
          variant="secondary"
          title={`${t("Sort")}: ${sortedOption}`}
        >
          <Dropdown.Item onClick={sortByNumberOfBooks}>
            {t("Number of Books")}
          </Dropdown.Item>
          <Dropdown.Item onClick={sortByNumberOfBooksAscending}>
            {t("Number of Books(Ascending)")}
          </Dropdown.Item>
          <Dropdown.Item onClick={sortAlphabetically}>
            {t("Alphabetically")}
          </Dropdown.Item>
          <Dropdown.Item onClick={sortAlphabeticallyReverse}>
            {t("Alphabetically(Reverse)")}
          </Dropdown.Item>
          <Dropdown.Item onClick={sortByBirthDate}>
            {t("By Oldest")}
          </Dropdown.Item>
          <Dropdown.Item onClick={sortByBirthDateReverse}>
            {t("By Youngest")}
          </Dropdown.Item>
        </DropdownButton>
      </ButtonGroup>
      ...
      <Offcanvas show={showFilterCanvas} onHide={handleCloseFilterCanvas}>
        ...
        <Offcanvas.Body>
          <FilterAuthorsForm
            setDisplayedAuthors={setDisplayedAuthors}
            allAuthors={startAuthorList}
            setCanvasShow={setShowFilterCanvas}
      ...
      {currentDisplayedAuthors.length ? (
        currentDisplayedAuthors.map((author) => {
          return (
                  ...
                  <Card.Body>
                    ...
                    <Card.Text>
                      <small className="text-muted">
                        {t("Number of Books")}: {author.numberOfBooks}
                      </small>
                    </Card.Text>
                    <Card.Text>
                      <small className="text-muted">
